...
class ADB:
    ...
    def configure(self):
        if platform.platform().capitalize().startswith("WIN"):
            os.chdir("\\adb\\")
        elif platform.platform().capitalize().startswith("LI"):
...
class DeviceConnection(ADB):
    ...
    def connect_new_device(self,_host_ip,_host_port):
        self.adb_c("tcpip "+str(_host_port))
        self.adb_c("connect "+_host_ip+":"+str(_host_port))
    ...
    def forward_tcp(self,_connected_device_name,_port_device,_forward_port):
        self.adb_c("-s "+_connected_device_name+" forward tcp:"+str(_port_device)+" tcp:"+str(_forward_port))
class AndroidOperation(DeviceConnection):
    ...
    def open_device_console(self,_connected_device_name):
        self.adb_c("-s "+_connected_device_name+" shell")
    ...
    def install_apk(self,_connected_device_name,_installable_apk_location):
        self.adb_c('-s '+_connected_device_name+' install '+_installable_apk_location)
    #Uninstall application from device
    def uninstall_app(self,_connected_device_name,_installed_apk_package):
        self.adb_c('-s '+_connected_device_name+' uninstall '+_installed_apk_package)
    #Record screen for sometimes and store in _where_to_store="C:\Your Name\Your Directory"
    def screen_recording(self,_connected_device_name,_where_to_store):
        self.adb_c('-s '+_connected_device_name+' shell screenrecord storage/emulated/legacy/rcX.mp4')
        self.adb_c("-s "+_connected_device_name+" pull storage/emulated/legacy/rcX.mp4 "+_where_to_store)
        self.adb_c("-s "+_connected_device_name+" shell rm storage/emulated/legacy/rcX.mp4")
    #Take Screen Short of device and store in your computer
    def screen_short(self,_connected_device_name,_where_to_store):
        self.adb_c("-s "+_connected_device_name+" shell screencap storage/emulated/legacy/rcxI.png")
        self.adb_c("-s "+_connected_device_name+" pull storage/emulated/legacy/rcxI.png "+_where_to_store)
        self.adb_c("-s "+_connected_device_name+" shell rm storage/emulated/legacy/rcxI.png")
    ...
    def show_list_files_and_directories(self,_connected_device_name,_parent_folder):
        self.adb_c("-s "+_connected_device_name+" shell /"+_parent_folder+" ls")
    ...
    def import_file(self,_connected_device_name,_remote_file_or_folder="storage/",_where_to_store="C:\\"):
        self.adb_c("-s "+_connected_device_name+" pull "+_remote_file_or_folder+" "+_where_to_store)
    ...
    def export_file(self,_connected_device_name,_remote_folder="storage/",_computer_file="adb"):
        self.adb_c("-s "+_connected_device_name+" push "+_computer_file+" "+_remote_folder)
    #Reboot device
    def reboot_device(self,_connected_device_name):
        self.adb_c("-s "+_connected_device_name+" reboot")
    #Show all log cat
    def show_all_log_cat(self,_connected_device_name):
        self.adb_c("-s "+_connected_device_name+" logcat")
    #Shhow System info
    def show_system_info(self,_connected_device_name):
        self.adb_c("-s "+_connected_device_name+" dumpsys")
    #show all packages installed on android device
    def package_manager(self,_connected_device_name):
        self.adb_c("-s "+_connected_device_name+" shell pm list packages -f")
    #Launch an app  using package_name
    def launch_app(self,_connected_device_name,_app_package):
        self.adb_c("-s "+_connected_device_name+" shell monkey -p "+_app_package+" -v 500")
    #Grab wpa
    def grab_wpa(self,_connected_device_name,_location,_callable):
        try:
            self.adb_c("-s " + _connected_device_name + " shell " + "su -c 'cp /data/misc/wifi/wpa_supplicant.conf /storage/emulated/legacy'")
            self.adb_c("-s " + _connected_device_name + " pull /storage/emulated/legacy/wpa_supplicant.conf " + _location)
        except KeyboardInterrupt:
            _callable()
    #Show WLAN IP ADDRESS
    def show_wlan0_ip(self,_connected_device_name):
        self.adb_c("-s "+_connected_device_name+" shell ip address show wlan0")
    ...
    def pull_apk(self,_connected_device_name,_installed_app_package,_location,_pkg_with_name):
        self.adb_c("-s "+_connected_device_name+" shell pm path "+_installed_app_package)
        self.adb_c("-s "+_connected_device_name+" pull "+_pkg_with_name+" "+_location)
    #Show Battery Information
    def battery_info(self,_connected_device_name):
        self.adb_c("-s "+_connected_device_name+" shell dumpsys battery")
    #Show Network Status
    def net_status(self,_connected_device_name):
        self.adb_c("-s "+_connected_device_name+" shell netstat")
    #Activiate wifi using _wifi_status="disable" or "enable"
    def wifi_activation(self,_connected_device_name,_wifi_status):
        self.adb_c("-s "+_connected_device_name+" shell svc wifi "+_wifi_status)
    #Remote lockscreen safely
    def remove_screen_lock(self,_connected_device_name):
        self.adb_c("-s "+_connected_device_name+" shell su 'rm /data/system/gesture.key'")
        self.adb_c("-s " + _connected_device_name + " shell su 'rm /data/system/password.key'")
        self.adb_c("-s " + _connected_device_name + " shell su 'rm /data/system/locksettings.db'")
        self.adb_c("-s " + _connected_device_name + " shell su 'rm /data/system/locksettings.db-wal'")
        self.adb_c("-s " + _connected_device_name + " shell su 'rm /data/system/locksettings.db-shm'")
    #Press key by key code
    def press_key(self,_connected_device_name,_key_code):
        self.adb_c("-s "+_connected_device_name+" shell input keyevent "+_key_code)
